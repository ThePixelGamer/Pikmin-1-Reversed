...
struct FaceNode : public CoreNode
{
    ...
    int* m_mtxIdx;       // 18h
    int* m_vtxIdx;       // 1Ch
    int* m_colIdx;       // 20h, assumed name
    int* m_nrmIdx;       // 24h
    int* m_texcoords[8]; // 28h
...
{
    int m_mapCode;
    // (m_mapCode >> 0x1b & 3) == slip code
    // (m_mapCode >> 0x1d) == attributes
    int dword4;
    int dword8;
    int dwordC;
    short word10;
...
struct BaseShape : public CoreNode
{
    ...
    int m_nbtCount;                           // _2D8
    Vector3f* m_nbt;                          // _2DC
    int m_attrListCount;                      // _2E0
    Texture** m_externalTextures;             // _2E4
    int m_attrListMatCount;                   // _2E8
...
struct StdSystemVTBL
{
    void (*initSoftReset)();
    RandomAccessStream* (*openFile)(char*, bool, bool);
    void (*copyRamToCache)(unsigned int, unsigned int, unsigned int);
    void (*copyCacheToRam)(unsigned int, unsigned int, unsigned int);
    void (*copyWaitUntilDone)();
    void (*copyCacheToTexture)(struct CacheTexture*);
    void (*Activate)(bool);
    void (*parseArchiveDirectory)(char*, char*);
    void (*sndPlaySe)(unsigned int);
    void (*startLoading)(struct LoadIdler*, bool, unsigned int);
    void (*endLoading)();
};

struct StdSystem
{
    StdSystemVTBL* vtbl;                // 0h
    float m_unk1;            // 4h
    float m_currentFade;     // 8h
    float m_startFade;       // Ch
    float m_endFade;         // 10h
    void* m_consFont;        // 14h, class Font
    int m_frameClamp;        // 18h
    int m_timerSetting;      // 1Ch
    int m_debugPrintSetting; // 20h
    int m_debugInfo;         // 24h
    int m_debugExtra;        // 28h
    int m_blurEnabled;       // 2Ch
    int m_fileInfo;          // 30h
    int m_drawColls;         // 34h
    void* m_drawTimer;       // 38h, class Timers
    ...
    void* dword19C;          // 19Ch
    void* dword1A0;          // 1A0h
    int m_renderedPolygons;  // 1A4h
    int m_renderedMats;      // 1A8h
    int m_renderedDisps;     // 1ACh
    int m_lightCount;        // 1B0h
    void* dword1B4;          // 1B4h
    int m_renderedAnims;     // 1B8h
    int m_lightSets;         // 1BCh
    void* dword1C0;          // 1C0h
    int m_renderedLightSets; // 1C4h
...
/*
if isStripped == false, then it is a fanned tri
*/
void DispList::outputTriData(RandomAccessStream& stream, int vcd, bool isStripped)
{
    for (FaceNode* i = this->m_face; i; i = i->next)
    {
        if (isStripped)
            stream.writeByte(stream, 0x98);
        else
            stream.writeByte(stream, 0xA0);

        stream.writeByte(stream, i->m_faceCount >> 8);
        stream.writeByte(stream, i->m_faceCount & 0xFF);

        for (int j = 0; j < i->m_faceCount; ++j)
        {
            if (i->m_mtxIdx)
            {
                if (vcd & 1)
                    stream.writeByte(3 * i->m_mtxIdx[j]);
                if (vcd & 2)
                    stream.writeByte(3 * i->m_mtxIdx[j] + 30);
            }
            else
            {
                if (vcd & 1)
                ...
                if (vcd & 2)
                    stream.writeByte(stream, 30);
            }

            stream.writeByte(stream, i->m_vtxIdx[j] >> 8);
            stream.writeByte(stream, i->m_vtxIdx[j] & 0xFF);

            if (i->m_nrmIdx)
            {
                stream.writeByte(stream, i->m_nrmIdx[j] >> 8);
                stream.writeByte(stream, i->m_nrmIdx[j] & 0xFF);
            }
            else
            {
                stream.writeByte(stream, 0);
            ...
            if (i->m_colIdx)
            {
                stream.writeByte(i->m_colIdx[j] >> 8);
                stream.writeByte(i->m_colIdx[j] & 0xFF);
            }

            bool foundTexCoord = false;
            for (int k = 0; k < 8; ++k) // TEXCOORD0 - TEXCOORD7
            {
                if (i->m_texcoords[k])
                {
                    stream.writeByte(i->m_texcoords[k][j] >> 8);
                    stream.writeByte(i->m_texcoords[k][j] & 0xFF);
                }
                foundTexCoord = true;
            }
            // Reading code will require to read 2 values regardless, so make them null
            if (foundTexCoord == false)
            {
                stream.writeByte(stream, 0);
                stream.writeByte(stream, 0);
            }
