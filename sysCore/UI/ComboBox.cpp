...
ComboBox::ComboBox(UIWindow *parent, int unk2, int style, int exstyle,
                   bool unk5)
...
void ComboBox::addOption(char *option, bool unsure) {
  ...
  LRESULT foundString = SendMessage(this->m_boxWindow->m_hWnd,
                                    LB_FINDSTRINGEXACT, NULL, (LPARAM)option);
  if (foundString != -1)
...
int ComboBox::processMessage(HWND hWnd, unsigned int Msg, WPARAM wParam,
                             long lParam) {
  if (wParam == WM_USER) {
    SendMessage(this->m_boxWindow->m_hWnd, WM_GETTEXT, sizeof(cBoxStringBuf),
                (LPARAM)cBoxStringBuf);
    this->entryHandler(cBoxStringBuf);

    LRESULT foundString =
        SendMessage(this->m_boxWindow->m_hWnd, CB_FINDSTRINGEXACT, 0xFFFFFFFF,
                    (LPARAM)cBoxStringBuf);
    if (foundString == -1)
      SendMessage(this->m_boxWindow->m_hWnd, CB_ADDSTRING, 0,
                  (LPARAM)cBoxStringBuf);
  }
...
void ComboBox::createWindow(LPCSTR className, LPCSTR windowName, HMENU hMenu) {
  ...
  this->m_boxWindow =
      new UIWindow(this, 15, this->m_boxExStyle | 0x50200000, NULL, NULL);
  ...
  ::lpPrevWndFunc =
      (WNDPROC)SetWindowLong(window, -4, (LPARAM)ComboBox_WndProc);
}