...
ToolWindow::ToolWindow(UIWindow *parent, int unk2, int style, int exstyle,
                       bool unk5)
    : UIWindow(parent, unk2, style, exstyle, unk5) {}

void ToolWindow::createWindow(LPCSTR className, LPCSTR windowName,
                              HMENU hMenu) {
...
void ToolWindow::initTools(HINSTANCE inst, int tickFreq, TBBUTTON *unkBut,
                           tagTBADDBITMAP *unkMap) {
  this->m_unk11 = inst;
  ...
  SendMessage(this->m_toolWindow->m_hWnd, TBM_SETTICFREQ, tickFreq,
              (LPARAM)unkBut);
  SendMessage(this->m_toolWindow->m_hWnd, TBM_GETBUDDY, 0, 0);
}

int ToolWindow::processMessage(HWND hWnd, unsigned int Msg, WPARAM wParam,
                               long lParam) {
  if (Msg == WM_NOTIFY && ((NMTTDISPINFOA *)lParam)->hdr.code == TTN_NEEDTEXT) {
    char buffer[512];
    LoadString(this->m_unk11, ((NMTTDISPINFOA *)lParam)->hdr.idFrom, buffer,
