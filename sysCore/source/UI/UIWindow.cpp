...
void UIWindow::refreshWindow() { this->updateSizes(this->m_client.width(), this->m_client.height()); }
...
HDWP UIWindow::resizeFrame(HDWP hWinPosInfo, RectArea& rect) { return hWinPosInfo; }
...
void UIWindow::createWindow(LPCSTR className, LPCSTR windowName, HMENU hMenu)
{
    ...
    this->m_hWnd =
        CreateWindowExA(this->m_dwExStyle, className, windowName, this->m_dwStyle, this->m_frame.x1, this->m_frame.y1,
                        this->m_frame.width(), this->m_frame.height(), hWndParent, this->m_hMenu, sysHInst, NULL);
