...
Colour::Colour(unsigned __int8 r, unsigned __int8 g, unsigned __int8 b, unsigned __int8 a) { this->set(r, g, b, a); }
...
void Colour::add(Colour& toAdd)
{
    ...
    unsigned short g = this->G + toAdd.G;
    if (g > 255)
        g = -1;
    else if (g < 0)
        g = 0;

    unsigned short b = this->B + toAdd.B;
    if (b > 255)
        b = -1;
    else if (b < 0)
        b = 0;
...
void Colour::genAge(AgeServer& server, char* unk) { server.NewEditor(unk, this); }
