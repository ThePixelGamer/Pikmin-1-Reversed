...
//cannot include both in header because of error LNK2001 aka circling dependency
...
bool AtxDirectRouter::openRoute(AtxStream* stream, int port) {
	...
	// if it can't connect to the host at port 1369, the route wasn't opened, close!
	...
	if (sysCurrWnd)
		this->m_networkStream->m_wSock->setASync(sysCurrWnd, 0x443, WM_MOUSEACTIVATE, -1);
...
void AtxDirectRouter::setWindow(unsigned __int32 hwnd) {
	if (hwnd)
		if (this->m_networkStream)
			this->m_networkStream->m_wSock->setASync((HWND)hwnd, 0x433, 0x21, SOCKET_ERROR);
}