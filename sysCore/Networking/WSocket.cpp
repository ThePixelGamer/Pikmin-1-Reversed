...
void WSocket::connect() {
  ...
  WSOCKETPRINT("got connected socket = %d : connection from %d.%d.%d.%d\n",
               this->m_acceptedSock, addr.sa_data[2], addr.sa_data[3],
  ...
  while (len) {
    ...
    if (recieved < 0) {
      ...
      if (lastError != WSAEWOULDBLOCK)
        WSOCKETHALT("recv on sock %d : error %d", this->m_acceptedSock,
                    lastError);
      WaitMessage();
      recieved = 0;
    }
...
void WSocket::setASync(HWND hWnd, unsigned __int32 wMsg,
                       unsigned __int32 lEvent, int sock) {
