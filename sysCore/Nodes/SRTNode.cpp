...
void SRTNode::update() {
  for (Node *i = static_cast<Node *>(this->child); i;
...
Matrix4f *SRTNode::getModelMatrix() { return &this->m_mat4f; }
...
void SRTNode::genAge(AgeServer &server) {
  ...
  this->m_srt.m_rotation.genAge(server, "R", RotationUpperBounds,
                                RotationLowerBounds);

  Vector3f TransformUpperBounds(1000.0f, 1000.0f, 1000.0f);
  Vector3f TransformLowerBounds(-1000.0f, -1000.0f, -1000.0f);
  this->m_srt.m_rotation.genAge(server, "T", TransformUpperBounds,
                                TransformLowerBounds);

  server.EndSection();
  this->genAge(server);
}

Vector3f *SRTNode::getPosition() { return &this->m_srt.m_position; }

Vector3f *SRTNode::getRotation() { return &this->m_srt.m_rotation; }

Vector3f *SRTNode::getScale() { return &this->m_srt.m_scale; }

Vector3f *SRTNode::getWorldPosition() {
  return reinterpret_cast<Vector3f *>(this->m_mat4f.mMatrix4f[3]);
}

void SRTNode::setPosition(Vector3f &pos) { this->m_srt.m_position = pos; }

void SRTNode::setRotation(Vector3f &rot) { this->m_srt.m_rotation = rot; }

void SRTNode::setScale(Vector3f &sca) { this->m_srt.m_scale = sca; }