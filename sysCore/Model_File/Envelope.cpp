...
void Envelope::read(RandomAccessStream& stream)
{
	...
	void* allocMem = operator new((sizeof(int) + sizeof(float)) * this->m_indicesCount);
	this->m_indices = static_cast<int*>(allocMem);
	// The weights section of the array starts after the indices, so set the array ptr to after the indices
