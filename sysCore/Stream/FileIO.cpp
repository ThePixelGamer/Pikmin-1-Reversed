...
void fileIO::write(void* buf, int count)
{
	fwrite(buf, 1, count, this->m_fStream);
}

int fileIO::getPending()
{ 
	return this->m_fileSize - this->m_fPtrPosition;
}

int fileIO::getAvailable()
{ 
...
void fileIO::setPosition(int offset)
{ 
	fseek(this->m_fStream, offset, SEEK_SET);
}
