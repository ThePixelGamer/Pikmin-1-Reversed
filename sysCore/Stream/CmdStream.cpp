...
void CmdStream::init(Stream *stream) {
  ...
  if (!CmdStream::statbuff) {
    CmdStream::statbuff =
        (char *)malloc(sizeof(char) * 0x8000); // or = new char[0x8000]
  }
...
void CmdStream::fillBuffer(bool canFill) {
  if (canFill || this->currentChar - this->unkVar3 >= 0x4000) {
    if (this->unkVar2) {
      memcpy(this->buffer + this->unkVar3,
             this->buffer + this->unkVar3 + 0x4000, 0x4000);
      this->buffer -= 0x4000;
      this->unkVar3 += 0x4000;
    }
...
char *CmdStream::getToken(bool hasComments) {
  ...
  while (true) {
    ...
    const signed int shouldBreak =
        tokenInParenthesis
            // if true
            ? this->buffer[currChar] == '"' || this->buffer[currChar] == '\''
                  ? 0
                  : 1
            // if false
