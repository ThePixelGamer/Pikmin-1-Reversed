...
void Module::Load(char *source) {
  ...
  if (this->pHInstance) {
    this->m_newObjAddr =
        (DLLFuncChar)GetProcAddress(this->pHInstance, "NewObject"); //(char)
    this->m_objListAddr =
        (DLLFuncVoid)GetProcAddress(this->pHInstance, "GetObjectList"); //(void)
    this->m_autoStartAddr =
        (DLLFuncVoid)GetProcAddress(this->pHInstance, "AutoStart"); //(void)
  } else {
    this->m_newObjAddr = 0;
